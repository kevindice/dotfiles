...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    ...
    elif [[ "`echo "$ZSH_HOST" | md5sum`" == "ubuntu" ]]; then
      prompt_segment 088 white "%(!.%{%F{yellow}%}.)$USER@%m"
    elif [[ "`echo "$ZSH_HOST" | md5sum`" == "e50569047556120d9485f0378d02a100  -" ]]; then
      prompt_segment 054 white "%(!.%{%F{yellow}%}.)$USER@%m"
    elif [[ "`echo "$ZSH_HOST" | md5sum`" == "b2ee672dd86391060fb0c6e6dfb1896c  -" ]]; then
